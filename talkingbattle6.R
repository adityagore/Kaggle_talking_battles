...
# 
X_full <- cbind(device_brand.matrix,device_models.matrix,device_app.matrix,device_categories_size.matrix,device_categories.matrix,device_weekday.matrix,device_day.matrix,device_week_day.matrix,device_week_day_activity.matrix)
...
pred <- predict(fit.glmnet,X_test,type="response",lambda=c(fit.glmnet.cv$lambda.min,fit.glmnet.cv$lambda.1se))
pred <- t(matrix(pred,nrow=length(levels(Y_full))))
colnames(pred) <- levels(Y_full)
first_submit <- data.frame(device_id=rownames(pred),pred[,,1])
colnames(first_submit) <- c("device_id",levels(Y_full))
write.csv(first_submit,file="submit1.0.csv",row.names=F,quote=F)
