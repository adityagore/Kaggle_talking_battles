...
events.apps.info[,':='(region=0L,regionLabel="NA")]

latLong <- events.apps.info[!is.na(longitude),.(longitude,latitude)] %>% unique()
setkeyv(latLong,c("longitude","latitude"))
latLong[,':='(region=0L,regionLabel="NA")]
...
for(REGION in unique(world.data$region)){
  ...
  print(REGION)
  print(paste0("Making spatial polygon: ",REGION))
  print(system.time(temp.region <- map2SpatialPolygons(map("world",REGION,fill = TRUE,plot=FALSE),proj4string=CRS("+proj=longlat +datum=WGS84"),IDs = rep(REGION,length(map("world",REGION,fill=TRUE,plot=FALSE)$names)))))
  print(paste0("Checking regions: ",REGION))
  print(system.time(latLong[regionLabel=="NA",region:=over(SpatialPoints(coords=data.frame(longitude,latitude),proj4string=CRS("+proj=longlat +datum=WGS84")),temp.region)]))
  expr <- parse(text=paste0("regionLabel:=\"",REGION,"\""))
  print(paste0("Adding regions: ",REGION))
  print(system.time(latLong[(region==1)&(regionLabel=="NA"),eval(expr)]))
  print(paste0("Done ",REGION))
  print(latLong[regionLabel!="NA"])
  gc()
  end.time <- Sys.time()
...
# events.apps.summary <- events.apps.info[,.(Nactivity=sum(is_active,na.rm = TRUE),Pactivity=mean(is_active,na.rm=TRUE),missingEvents=max(missingEvents),missingApps=max(missingApps),location=paste0(regionLabel_over,collapse = ","),region=any(region)),by=.(device_id,weekday,partofday,app_id,categories,timeday)]
...
events.apps.info[,week_day:=paste0(as.character(weekday),":",as.character(partofday))]
gc()
# device.categories <- events.apps.summary[!is.na(app_id),.(N=sum(Nactivity),P=mean(Pactivity)),by=.(device_id,app_id,categories)]
device.categories <- events.apps.info[!is.na(app_id),.(Nactivity=sum(is_active,na.rm=TRUE),Pactivity=mean(is_active,na.rm=TRUE)),by=.(device_id,app_id,categories)]
gc()
...
device.categories.length <- device.categories[,.(freq=sum(Nactivity)),by=.(device_id,app.group)]


categories <- strsplit(device.categories[,categories],",")
device.categories.new <- data.table("device_id"=rep(device.categories[,device_id],unlist(lapply(categories,length))),"Nactivity"=rep(device.categories[,Nactivity],unlist(lapply(categories,length))),"Pactivity"=rep(device.categories[,Pactivity],unlist(lapply(categories,length))),"categories"=unlist(categories))

device.categories <- device.categories.new[,.(Nactivity=sum(Nactivity),Pactivity=mean(Pactivity)),by=.(device_id,categories)]
rm(device.categories.new)
gc()

# device.categories.obs <- events.apps.info[,.N,by=.(device_id,categories)]
# device.categories.obs[grepl("unknown,unknown",categories),categories:=gsub("unknown,unknown","unknown",categories)]
# categories <- strsplit(device.categories.obs[,categories],",")
# device.categories.obs <- data.table("device_id"=rep(device.categories.obs[,device_id],unlist(lapply(categories,length))),"N"=rep(device.categories.obs[,N],unlist(lapply(categories,length))),"categories"=unlist(categories))
# device.categories.obs <- device.categories.obs[,.(N=sum(N)),by=.(device_id,categories)]

# device.categories <- unique(device.categories)
# device.categories[,Pcategories:=Ncategories/sum(Ncategories),by=device_id]

device.weekday.activities <- events.apps.info[!is.na(weekday),.(Nactivity=sum(is_active,na.rm=TRUE),Pactivity=mean(is_active,na.rm=TRUE)),by=.(device_id,weekday)]
gc()
device.partofday.activities <- events.apps.info[!is.na(partofday),.(Nactivity=sum(is_active,na.rm=TRUE),Pactivity=mean(is_active,na.rm=TRUE)),by=.(device_id,partofday)]
gc()
device.week_day.activities <- events.apps.info[!is.na(weekday),.(Nactivity=sum(is_active,na.rm=TRUE),Pactivity=mean(is_active,na.rm=TRUE)),by=.(device_id,week_day)]

gc()

device.events <- events.apps.info[,.(missingEvents=max(missingEvents)),by=device_id]

device.app.activities <- events.apps.info[!is.na(app_id),.(Nactivity=sum(is_active,na.rm=TRUE),Pactivity=mean(is_active,na.rm=TRUE)),by=.(device_id,app_id)]
device.missing.apps <- events.apps.info[,.(missingApps=max(missingApps,na.rm=TRUE)),by=device_id]

gc()

device.locations <- events.apps.info[!is.na(regionLabel_over),.(Nactivity=sum(is_active,na.rm = TRUE),Pactivity=mean(is_active,na.rm=TRUE)),by=.(device_id,regionLabel_over)]

rm(events.apps.info)
gc()

unique_devices <- total.info[,device_id] %>% unique()
# unique_devices_with_events <- events.apps.summary[missingEvents==0,device_id] %>% unique()
# unique_mapps <- events.apps.summary[missingEvents==0,app_id] %>% unique()
unique_apps <- device.app.activities[!is.na(app_id),app_id] %>% unique()
unique_brands <- total.info[,phone_brand] %>% unique()
unique_models <- total.info[,device_model] %>% unique()
unique_weekday <- device.weekday.activities[,weekday]%>%unique()
unique_day <- device.partofday.activities[,partofday]%>%unique()
unique_week_day <- device.week_day.activities[,week_day] %>% unique()
unique_categories <- device.categories[,categories]%>% unique()
unique_app_group <- device.categories.length[,app.group] %>% unique()
unique_locations <- device.locations[,regionLabel_over] %>% unique()
...
X_brand_full <- sparseMatrix(i=id_devices,j=id_brands,x=1,dimnames = list(unique_devices,paste("Brand: ",unique_brands)))
X_models_full <- sparseMatrix(i=id_devices,j=id_models,x=1,dimnames = list(unique_devices,paste("Model: ",unique_models)))

id_devices <- match(device.app.activities[,device_id],unique_devices)
id_apps <- match(device.app.activities[,app_id],unique_apps)

X_apps_full <- sparseMatrix(i=id_devices,j=id_apps,x=1,dimnames = list(unique_devices,paste("App ID: ",unique_apps)),dims = c(length(unique_devices),length(unique_apps)))
X_apps_activities_full.log <- sparseMatrix(i=id_devices,j=id_apps,x=log1p(device.app.activities[,Nactivity]),dimnames = list(unique_devices,paste("App Activity Log: ",unique_apps)),dims = c(length(unique_devices),length(unique_apps)))
X_apps_activities_full <- sparseMatrix(i=id_devices,j=id_apps,x=(device.app.activities[,Nactivity]),dimnames = list(unique_devices,paste("App Activity: ",unique_apps)),dims = c(length(unique_devices),length(unique_apps)))
X_apps_activities_full.P <- sparseMatrix(i=id_devices,j=id_apps,x=(device.app.activities[,Pactivity]),dimnames = list(unique_devices,paste("App Activity P: ",unique_apps)),dims = c(length(unique_devices),length(unique_apps)))
X_apps_activities_full.Pm <- sparseMatrix(i=id_devices,j=id_apps,x=expm1(device.app.activities[,Pactivity]),dimnames = list(unique_devices,paste("App Activity Pm: ",unique_apps)),dims = c(length(unique_devices),length(unique_apps)))



id_devices <- match(device.categories.length[,device_id],unique_devices)
id_app_groups <- match(device.categories.length[,app.group],unique_app_group)

X_app_groups_full <- sparseMatrix(i=id_devices,j=id_app_groups,x=1,dimnames = list(unique_devices,paste("Group: ",unique_app_group)),dims = c(length(unique_devices),length(unique_app_group)))
X_app_groups_activities_full <- sparseMatrix(i=id_devices,j=id_app_groups,x=(device.categories.length[,freq]),dimnames = list(unique_devices,paste("Group Activity: ",unique_app_group)),dims = c(length(unique_devices),length(unique_app_group)))
X_app_groups_activities_full.log <- sparseMatrix(i=id_devices,j=id_app_groups,x=log1p(device.categories.length[,freq]),dimnames = list(unique_devices,paste("Group Activity Log: ",unique_app_group)),dims = c(length(unique_devices),length(unique_app_group)))
...
X_categories_full <- sparseMatrix(i=id_devices,j=id_categories,x=1,dimnames = list(unique_devices,paste("Category: ",unique_categories)),dims = c(length(unique_devices),length(unique_categories)))
X_categories_activities_full <- sparseMatrix(i=id_devices,j=id_categories,x=device.categories[,Nactivity],dimnames = list(unique_devices,paste("Category Activity: ",unique_categories)),dims = c(length(unique_devices),length(unique_categories)))
X_categories_activities_full.log <- sparseMatrix(i=id_devices,j=id_categories,x=log1p(device.categories[,Nactivity]),dimnames = list(unique_devices,paste("Category Activity Log: ",unique_categories)),dims = c(length(unique_devices),length(unique_categories)))
X_categories_activities_full.P <- sparseMatrix(i=id_devices,j=id_categories,x=device.categories[,Pactivity],dimnames = list(unique_devices,paste("Category Activity P: ",unique_categories)),dims = c(length(unique_devices),length(unique_categories)))
X_categories_activities_full.Pm <- sparseMatrix(i=id_devices,j=id_categories,x=expm1(device.categories[,Pactivity]),dimnames = list(unique_devices,paste("Category Activity Pm: ",unique_categories)),dims = c(length(unique_devices),length(unique_categories)))
...
X_weekdays_full <- sparseMatrix(i=id_devices,j=id_weekdays,x=1,dimnames = list(unique_devices,paste("Weekday: ",unique_weekday)),dims = c(length(unique_devices),length(unique_weekday)))
X_weekdays_activities_full <- sparseMatrix(i=id_devices,j=id_weekdays,x=device.weekday.activities[,Nactivity],dimnames = list(unique_devices,paste("Weekday Activity: ",unique_weekday)),dims = c(length(unique_devices),length(unique_weekday)))
X_weekdays_activities_full.log <- sparseMatrix(i=id_devices,j=id_weekdays,x=log1p(device.weekday.activities[,Nactivity]),dimnames = list(unique_devices,paste("Weekday Activity Log: ",unique_weekday)),dims = c(length(unique_devices),length(unique_weekday)))
X_weekdays_activities_full.P <- sparseMatrix(i=id_devices,j=id_weekdays,x=device.weekday.activities[,Pactivity],dimnames = list(unique_devices,paste("Weekday Activity P: ",unique_weekday)),dims = c(length(unique_devices),length(unique_weekday)))
X_weekdays_activities_full.Pm <- sparseMatrix(i=id_devices,j=id_weekdays,x=expm1(device.weekday.activities[,Pactivity]),dimnames = list(unique_devices,paste("Weekday Activity Pm: ",unique_weekday)),dims = c(length(unique_devices),length(unique_weekday)))
...
X_partofdays_full <- sparseMatrix(i=id_devices,j=id_partofdays,x=1,dimnames = list(unique_devices,paste("PD: ",unique_day)),dims = c(length(unique_devices),length(unique_day)))
X_partofdays_activities_full <- sparseMatrix(i=id_devices,j=id_partofdays,x=device.partofday.activities[,Nactivity],dimnames = list(unique_devices,paste("PD Activity: ",unique_day)),dims = c(length(unique_devices),length(unique_day)))
X_partofdays_activities_full.log <- sparseMatrix(i=id_devices,j=id_partofdays,x=log1p(device.partofday.activities[,Nactivity]),dimnames = list(unique_devices,paste("PD Activity Log: ",unique_day)),dims = c(length(unique_devices),length(unique_day)))
X_partofdays_activities_full.P <- sparseMatrix(i=id_devices,j=id_partofdays,x=device.partofday.activities[,Pactivity],dimnames = list(unique_devices,paste("PD Activity P: ",unique_day)),dims = c(length(unique_devices),length(unique_day)))
X_partofdays_activities_full.Pm <- sparseMatrix(i=id_devices,j=id_partofdays,x=expm1(device.partofday.activities[,Pactivity]),dimnames = list(unique_devices,paste("PD Activity Pm: ",unique_day)),dims = c(length(unique_devices),length(unique_day)))
...
X_week_days_full <- sparseMatrix(i=id_devices,j=id_week_days,x=1,dimnames = list(unique_devices,paste("Week.day: ",unique_week_day)),dims = c(length(unique_devices),length(unique_week_day)))
X_week_days_activities_full <- sparseMatrix(i=id_devices,j=id_week_days,x=device.week_day.activities[,Nactivity],dimnames = list(unique_devices,paste("Week.day Activity: ",unique_week_day)),dims = c(length(unique_devices),length(unique_week_day)))
X_week_days_activities_full.log <- sparseMatrix(i=id_devices,j=id_week_days,x=log1p(device.week_day.activities[,Nactivity]),dimnames = list(unique_devices,paste("Week.day Activity Log: ",unique_week_day)),dims = c(length(unique_devices),length(unique_week_day)))
X_week_days_activities_full.P <- sparseMatrix(i=id_devices,j=id_week_days,x=device.week_day.activities[,Pactivity],dimnames = list(unique_devices,paste("Week.day Activity P: ",unique_week_day)),dims = c(length(unique_devices),length(unique_week_day)))
X_week_days_activities_full.Pm <- sparseMatrix(i=id_devices,j=id_week_days,x=expm1(device.week_day.activities[,Pactivity]),dimnames = list(unique_devices,paste("Week.day Activity Pm: ",unique_week_day)),dims = c(length(unique_devices),length(unique_week_day)))

id_locations <- match(device.locations[,regionLabel_over],unique_locations)
id_devices <- match(device.locations[,device_id],unique_devices)

X_locations_full <- sparseMatrix(i=id_devices,j=id_locations,x=1,dimnames = list(unique_devices,paste("Location:",unique_locations)),dims = c(length(unique_devices),length(unique_locations)))
X_locations_activities_full <- sparseMatrix(i=id_devices,j=id_locations,x=device.locations[,Nactivity],dimnames = list(unique_devices,paste("Location Activity:",unique_locations)),dims = c(length(unique_devices),length(unique_locations)))
X_locations_activities_full.log <- sparseMatrix(i=id_devices,j=id_locations,x=log1p(device.locations[,Nactivity]),dimnames = list(unique_devices,paste("Location Activity Log:",unique_locations)),dims = c(length(unique_devices),length(unique_locations)))
X_locations_activities_full.P <- sparseMatrix(i=id_devices,j=id_locations,x=device.locations[,Pactivity],dimnames = list(unique_devices,paste("Location Activity P:",unique_locations)),dims = c(length(unique_devices),length(unique_locations)))
X_locations_activities_full.Pm <- sparseMatrix(i=id_devices,j=id_locations,x=expm1(device.locations[,Pactivity]),dimnames = list(unique_devices,paste("Location Activity Pm:",unique_locations)),dims = c(length(unique_devices),length(unique_locations)))
...
matrixNames <- c("X_brand_full"=TRUE,
                 "X_models_full"=TRUE,
                 "X_apps_full"=TRUE,
                 "X_apps_activities_full"=FALSE,
                 "X_apps_activities_full.log"=FALSE,
                 "X_apps_activities_full.P"=FALSE, 
                 "X_apps_activities_full.Pm"=FALSE,
                 "X_app_groups_full"=TRUE,
                 "X_app_groups_activities_full"=FALSE,
                 "X_app_groups_activities_full.log"=TRUE,
                 "X_categories_full"=TRUE,
                 "X_categories_activities_full"=FALSE,
                 "X_categories_activities_full.log"=FALSE,
                 "X_categories_activities_full.P"=FALSE,
                 "X_categories_activities_full.Pm"=FALSE,
                 "X_partofdays_full"=TRUE,
                 "X_partofdays_activities_full"=FALSE,
                 "X_partofdays_activities_full.log"=TRUE,
                 "X_partofdays_activities_full.P"=FALSE,
                 "X_partofdays_activities_full.Pm"=FALSE,
                 "X_week_days_full"=FALSE,
                 "X_week_days_activities_full"=FALSE,
                 "X_week_days_activities_full.log"=FALSE,
                 "X_week_days_activities_full.P"=FALSE,
                 "X_week_days_activities_full.Pm"=FALSE,
                 "X_weekdays_full"=TRUE,
                 "X_weekdays_activities_full"=FALSE,
                 "X_weekdays_activities_full.log"=FALSE,
                 "X_weekdays_activities_full.P"=FALSE,
                 "X_weekdays_activities_full.Pm"=FALSE,
                 "X_locations_full"=TRUE,
                 "X_locations_activities_full"=FALSE,
                 "X_locations_activities_full.log"=FALSE,
                 "X_locations_activities_full.P"=FALSE,
                 "X_locations_activities_full.Pm"=FALSE,
                 "X_missingEvents"=TRUE,
                 "X_missingApps"=FALSE)
...
indexTrain <- createDataPartition(Y_train,p=0.8,list=FALSE)

X_train_train <- X_train[indexTrain[,1],]
X_train_test <- X_train[-indexTrain[,1],]

Y_train_train <- Y_train[indexTrain[,1]]
Y_train_test <- Y_train[-indexTrain[,1]]

first_control <- trainControl(
  method="repeatedcv",
  number=3,
  repeats=5,
  ...
  # index = createResample(Y_train,25),
  allowParallel = FALSE
)
levels(Y_train_train)<-make.names(levels(Y_train_train))
gbm_model <- train(x=X_train_train,y=Y_train_train,
                   method="gbm",
                   metric = "logLoss",
                   maximize = FALSE,
                   trControl = first_control)
...
write.csv(first_submit,file="submit1.3.csv",row.names=F,quote=F)
